// api/fetch-feed.js — vrátí 20 nejnovějších článků z RSS feedu aktualne.cz
// Server-side (bez CORS). Frontend volá /api/fetch-feed (same-origin → connect-src 'self').

const FEED_URL = "https://www.aktualne.cz/rss";
...
export default async function handler(req, res) {
  ...
  while ((m = re.exec(xml)) && items.length < 20) {
    ...
    try {
      if (ART_HOST.test(new URL(link).hostname) && title) {
